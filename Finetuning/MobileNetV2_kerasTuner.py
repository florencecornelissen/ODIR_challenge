...
from keras.applications import ResNet50, VGG19, DenseNet201, MobileNetV2
...
# checkpoint_directory = f'./CheckpointMobileNetV2_kerasTuner/'

# if not os.path.exists(checkpoint_directory):
#     os.mkdir(checkpoint_directory)

# checkpoint_filepath = './CheckpointMobileNetV2_kerasTuner'
...
# tuner = kt.Hyperband(build_model, objective='val_accuracy', max_epochs=20, factor=3, directory='MobileNetV2tunerdirectory', project_name='ODIR_challenge')


# tuner.search(X_train, y_train, epochs=50, validation_split=500/len(X_train), callbacks=[model_checkpoint_callback, early_stopping_callback])

# best_hps=tuner.get_best_hyperparameters(num_trials=1)[0]

# print(f"""
# The hyperparameter search is complete for MobileNetV2. The optimal number of units in the first densely-connected
# layer is {best_hps.get('units')}, the optimal learning rate for the optimizer
# is {best_hps.get('learning_rate')}, and the optimal dropout is {best_hps.get('dropout')}.
# """)
...
model = load_model('./CheckpointMobileNetV2_kerasTuner/')
model.compile(optimizer='adam', loss='binary_crossentropy', metrics=['AUC', 'accuracy'])
...
testdf.to_csv('submissionMobileNetV2_kerasTuner.csv', index=False)
