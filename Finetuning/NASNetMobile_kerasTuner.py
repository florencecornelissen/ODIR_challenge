...
from keras.applications import ResNet50, VGG19, NASNetMobile
...
checkpoint_directory = f'./CheckpointNASNetMobile_kerasTuner/'

if not os.path.exists(checkpoint_directory):
    os.mkdir(checkpoint_directory)

checkpoint_filepath = './CheckpointNASNetMobile_kerasTuner'
...
tuner = kt.Hyperband(build_model, objective='val_accuracy', max_epochs=20, factor=3, directory='NASNetMobiletunerdirectory', project_name='ODIR_challenge')

tuner.search(X_tune, y_tune, epochs=50, validation_split=0.1, callbacks=[model_checkpoint_callback, early_stopping_callback])

best_hps=tuner.get_best_hyperparameters(num_trials=1)[0]

print(f"""
The hyperparameter search for NASNetMobile is complete. The optimal number of units in the first densely-connected
...
model = load_model('./CheckpointNASNetMobile_kerasTuner/')
model.compile(optimizer='adam', loss='binary_crossentropy', metrics=['AUC', 'accuracy'])
...
testdf.to_csv('submissionNASNetMobile_kerasTuner.csv', index=False)
