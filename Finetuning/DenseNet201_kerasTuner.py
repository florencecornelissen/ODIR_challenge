...
checkpoint_directory = f'./CheckpointDenseNet201_kerasTuner/'

if not os.path.exists(checkpoint_directory):
    os.mkdir(checkpoint_directory)

checkpoint_filepath = './CheckpointDenseNet201_kerasTuner'
...
tuner = kt.Hyperband(build_model, objective='val_accuracy', max_epochs=20, factor=3, directory='DenseNet201tunerdirectory', project_name='ODIR_challenge')
...
print(f"""
The hyperparameter search for DenseNet201 is complete. The optimal number of units in the first densely-connected
...
model = load_model('./CheckpointDenseNet201_kerasTuner/')
model.compile(optimizer='adam', loss='binary_crossentropy', metrics=['AUC', 'accuracy'])
...
testdf.to_csv('submissionDenseNet201_kerasTuner.csv', index=False)
